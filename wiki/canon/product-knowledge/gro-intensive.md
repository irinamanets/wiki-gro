---
id: mkt:canon/product-knowledge/gro-intensive
title: GRO «Интенсив» — second monetization track (high-ticket B2B/cohort)
type: page
subtype: concept
layer: canon
theme: product-knowledge
tags: [product, pricing, b2b, landing-page]
confidence: medium
stale: false
created: 2026-05-16
updated: 2026-05-19  # +чекаут «Тарифа 1»: тот же ценовой набор (240k/400k), что у «Тарифа 2» → тарифы различаются НЕ ценой; закрыт open question #2
sources: [sources/2026-05-16-groapp-payment-intensive-tarif2.md, sources/2026-05-18-pressfeed-ai-intensives-overview.md, sources/2026-05-19-pressfeed-kakie-kursy-novichku-baranova-placement.md, sources/2026-05-19-pressfeed-ai-tools-for-online-courses-15.md, sources/2026-05-19-groapp-payment-intensive-tarif1.md]
namespace: mkt
---

# GRO «Интенсив» — second monetization track

Помимо подписочного мобильного/веб-приложения (см. [[canon/product-knowledge/gro-app-overview]] и [[canon/product-knowledge/gro-pricing]] про 2 490 ₽/мес), у GRO существует **второй продуктовый трек — «Интенсив»**. Это high-ticket B2B/cohort-формат, обслуживаемый отдельными checkout-страницами на `groapp.ru/payment-intensive-tarif{N}`. Зафиксировано два чекаут-источника — «Тариф 1» (см. [[sources/2026-05-19-groapp-payment-intensive-tarif1]]) и «Тариф 2» (см. [[sources/2026-05-16-groapp-payment-intensive-tarif2]]) — и оба несут **идентичную ценовую структуру** (240 000 / 400 000 ₽), то есть тарифные страницы серии различаются НЕ ценой (см. ниже).

**Статус знания:** `confidence: medium`. Что такое интенсив **содержательно** (программа, длительность, формат — онлайн/офлайн), на момент 2026-05-16 не раскрыто в имеющихся источниках. Известна только тарифная и checkout-механика. Структурные факты `confidence: high`, интерпретация модели — `confidence: medium`.

## Что зафиксировано

### Структура тарифа

URLs `groapp.ru/payment-intensive-tarif1` и `groapp.ru/payment-intensive-tarif2` указывают на **серию тарифных страниц** (`tarif1`, `tarif2`, и потенциально дальше). На 2026-05-19 заингесчены **обе** страницы, и обе несут **одинаковый ценовой набор**:

| Вариант | Цена (Тариф 1) | Цена (Тариф 2) | Семантика |
|---|---|---|---|
| «Иду один» | **240 000 ₽** | **240 000 ₽** | Соло-участник, один seat |
| «Иду с директором по маркетингу» | **400 000 ₽** | **400 000 ₽** | Founder + CMO bundle (2 seat'а) |
| (служебный) «Тест = 1 ₽» | 1 ₽ | 1 ₽ | Тестовый вариант платёжного флоу, не маркетинговый offer |

Цифры «Тарифа 1» — `[conf:high, src:2026-05-19]`, «Тарифа 2» — `[conf:high, src:2026-05-16]` (обе first-party страницы GRO).

**Корректировка модели (2026-05-19):** прежняя гипотеза «продуктовая команда сегментировала интенсив на ценовые уровни (базовый/расширенный/премиум)» **не подтвердилась**. Цены на `tarif1` и `tarif2` совпадают до рубля, значит тарифные страницы различаются **не ценой**, а чем-то иным: поток/дата когорты, конкретный набор интенсива, либо это просто две checkout-страницы для одного оффера (разные UTM/каналы трафика). Что именно различается — пока не раскрыто (см. backlog).

### Экономика bundle

Дельта за второго участника: **+160 000 ₽** (400 000 − 240 000). По сравнению с «два места по 240k = 480 000 ₽» это **скидка ~33% на seat-2**. Это устойчивый паттерн в team-cohort-программах: первый seat по полной, второй — со скидкой, чтобы убрать ценовой барьер для покупателя-команды.

### Buyer pattern: founder + CMO

Формулировка варианта 2 — «Иду **с директором по маркетингу**» — это самый сильный single-signal маркер ICP интенсива:

- Покупатель — **владелец/CEO** (тот, кто «идёт»)
- Он берёт **функциональную правую руку по маркетингу** (своего CMO/директора по маркетингу/маркетолога-руководителя)
- Это **SMB B2B** (в крупном корпорате обучение CMO согласовывается через HR/L&D, а не покупается founder'ом из своих)
- Маркетинговая функция уже **выделена в роль** — то есть это не founder-solo, а компания минимум с одним менеджером, отвечающим за маркетинг

Формат позиционирования прямо называет должность («директор по маркетингу»), а не нейтральное «коллега» — это speaks-to-segment: founder читает свою ситуацию буквально, а не интерпретирует.

### Сила сегмент-сигнала vs ограничения

Что **достоверно**:
- Целевой покупатель — owner SMB с уже выделенной маркетинговой функцией
- Цена интенсива на порядки выше подписки (240 000 ₽ vs ~2 490 ₽/мес — 96× за разовый offer)
- Это premium / high-ticket трек, не volume-play

Что **не достоверно** (требует verify):
- Формат («интенсив» в РФ — общее слово, может быть: live-cohort, 1-on-1, mastermind, конференция, retreat, intensive-курс)
- Длительность, регулярность
- Что входит в цену (контент, доступ к сообществу, личные сессии, материалы)

### Form & checkout

Чекаут собирает **6 обязательных полей**:
1. Email
2. Имя
3. Фамилия
4. Телефон с маской `+7 (999) 999-9999` (РФ-only сегмент покупателей)
5. Telegram-handle (отдельное от телефона поле — то есть Telegram считается основным каналом коммуникации с участниками)
6. Чекбокс согласия с [политикой конфиденциальности](https://groapp.ru/privacy) и [офертой](https://groapp.ru/oferta)

**Брендовый акцент** в ссылках — `rgb(255, 40, 180)` (магента) — согласуется с identity GRO на основном лендинге.

## Импликации для маркетинга

### Двухконтурная продуктовая архитектура

GRO держит **two tracks монетизации**, которые работают на разные аудитории и разные ценовые сегменты:

1. **Low-ticket subscription** — приложение/веб, 83 ₽/день (2 490 ₽/мес), trial «14 дней за 1 ₽». ICP — все три сегмента из [[canon/target-audience/gro-segments]] (карьеристы, предприниматели в росте, фрилансеры), broad funnel.
2. **High-ticket intensive** — 240 000–400 000 ₽ за тариф. ICP — owner SMB с маркетинговой функцией, narrow funnel, premium-cohort.

Контентный funnel логично укладывается: подписка обслуживает awareness/consideration broad-funnel, интенсив — конверсия premium-сегмента. Между ними скорее всего lead-nurturing через канал и продукт.

### Что это говорит про канал @gro_me и founder-content

Канал [[sources/2026-05-12-tg-gro-me-channel-dump|@gro_me]] и founder-контент Лапшиной — могут работать **именно на интенсив-сегмент** (а не только на подписку), потому что:
- Owner-аудитория читает founder-контент как peer-content
- High-ticket требует доверия к фигуре, не только к продукту
- В подписочном flow founder-фигура — bonus, в интенсиве — необходимое условие конверсии

Это можно отдельно тестировать в [[evolving/content-trends/founder-expert-hook-family-vcru]] и связанных страницах: какие хуки лучше работают на retention подписки vs на конверсию в интенсив.

### Антипаттерны контентной подачи

- **Не миксовать** цены подписки и интенсива в одном креативе. «От 83 ₽/день до 400 000 ₽ за интенсив» — это разные офферы для разной аудитории, mix ломает обе подачи.
- **Не использовать** «240 000 ₽» как anchor в контенте broad-funnel (подписка) — высокая цена отпугнёт low-ticket-аудиторию ещё до того, как она поймёт, что это другой продукт.
- **Не цитировать** структуру тарифа («2 тарифа», «тариф 2 с двумя вариантами») вслух — внутренняя сегментная логика, не маркетинговый offer.

## Открытые вопросы (backlog)

См. также раздел «Что неизвестно» в [[sources/2026-05-16-groapp-payment-intensive-tarif2]] и [[sources/2026-05-19-groapp-payment-intensive-tarif1]].

1. **Что такое интенсив содержательно** — нужен ingest описательной страницы (вероятно `groapp.ru/intensive` или связанной).
2. ~~**Иерархия тарифов** — что есть в «Тарифе 1», существуют ли «Тариф 3+».~~ **Частично закрыто (2026-05-19):** «Тариф 1» содержит тот же ценовой набор, что «Тариф 2» (240k/400k). Тарифы различаются **не ценой**. Остаётся открытым: чем именно они различаются (поток/дата/состав/трафик-канал) и существуют ли `tarif3+`.
3. **Регулярность** — разовый набор или периодический поток (cohort-based). Возможно, что `tarif1`/`tarif2` — это два **потока/даты** одного оффера, что бы подтвердило cohort-модель.
4. **Что значит «иду с директором по маркетингу» физически** — два живых seat'а на программу, или один seat + материалы/lo-touch для CMO покупателя.
5. **Связка с подпиской** — что происходит с подпиской у покупателя интенсива (включена в цену? нет? upgrade-path?).
6. **Команда, ведущая интенсив** — Лапшина? Тарасов? приглашённые эксперты? — не раскрыто.

## Landscape positioning (vs RU-AI-intensives massовый рынок)

Pressfeed-обзор «Интенсивы по нейросетям» (2026-05-18, см. [[sources/2026-05-18-pressfeed-ai-intensives-overview]]) фиксирует **массовую категорию AI-интенсивов** в РФ — школа Барановой, Skillbox, Академия Эдюсон, ряд безымянных школ под фриланс/удалёнку. Полный landscape — в [[evolving/industry-trends/ru-ai-intensive-courses-landscape-2026]].

GRO «Интенсив» **не конкурирует напрямую** с этой массовой категорией. Ключевые разграничители:

| Параметр | Mass-market AI-intensive | GRO «Интенсив» |
|---|---|---|
| Цена | ~5 000 – 80 000 ₽ (low/mid-ticket) | 240 000 – 400 000 ₽ (high-ticket) |
| Buyer | B2C-prosumer (специалист, фрилансер) | B2B-owner + CMO (компания минимум с маркетинговой функцией) |
| Промис | «Овладеть инструментами AI» | (требует verify, но из buyer-pattern) — «Прокачать маркетинг бизнеса через AI вместе с CMO» |
| Длительность | 2 дня – 2 недели | (не раскрыто) |
| Takeaway | Промпты, шаблоны, базовая автоматизация | (вероятно) маркетинговая стратегия + операционный план для команды |
| Конкуренты по нише | Skillbox AI, Эдюсон, школа Барановой | [[canon/marketing-frameworks/spiridonov-three-engagement-formats|Стратегическая сессия Спиридонова]], бизнес-клубы типа [[evolving/competitor-positioning/business-booster-visotsky|Business Booster]], корпоративные программы EdTech |

**Implication для коммуникации:** Когда аудитория ищет «AI-интенсив 2026», она по умолчанию находится в **mass-market frame** (Skillbox, Эдюсон, школа Барановой). Если GRO «Интенсив» попадает в эту выдачу как «один из» — высока вероятность mismatch (цена 240k+ ₽ vs ожидание 20–80k ₽). **Корректный канал размещения** — каналы, где живёт founder+CMO-аудитория (не «обучающие платформы», а «бизнес-сообщества для предпринимателей с командой»). Это совпадает с [[canon/target-audience/gro-segments|Сегмент 2: предприниматели в росте]] sub-segment с маркетинговой функцией.

**Применение 5 критериев** ([[canon/marketing-frameworks/ai-intensive-format-criteria]]) к GRO «Интенсиву»:
- **Критерий 1 (практика в каждом блоке)** — для high-ticket это **глубокая работа на собственном бизнесе**, не «попробовали Midjourney».
- **Критерий 2 (чёткий фокус)** — фокус GRO «Интенсива» должен быть на **бизнес-маркетинге через AI**, не на «обзоре AI-инструментов».
- **Критерий 3 (актуальные сервисы)** — должны быть в программе, но это floor, не differentiator. **Operational benchmark:** покрытие ≥5 из 7 этапов конвейера ([[canon/marketing-frameworks/ai-course-production-conveyor-7-stages]]) — необходимый минимум, иначе провал критерия 3 по Pressfeed-mass-medium-стандарту 2026. Inventory конкретных RU-инструментов — в [[evolving/content-trends/ru-ai-course-production-stack-2026]].
- **Критерий 4 (разбор результатов)** — для high-ticket это **личный feedback от founder-команды**, не AI-проверка.
- **Критерий 5 (takeaway-сценарий)** — должен быть на уровне **операционного маркетингового плана**, а не «5 промптов».

**Применение 3 differentiators** ([[canon/marketing-frameworks/beginner-edu-3-differentiators]]) к GRO «Интенсиву» — diagnostic «является ли это beginner-форматом»:

| Differentiator | GRO «Интенсив» проходит? | Implication |
|---|---|---|
| 1. Работа со страхами | **Нет** | Целевой клиент уже преодолел барьер «AI слишком сложно» (он уже founder с CMO, у него есть маркетинговая функция). Адресация страхов irrelevant. |
| 2. Быстрый первый результат | **Условно** | Артефакт «попробовать инструмент в 1 сессии» не релевантен. Релевантен «получить готовую стратегию в 2–3 дня». Это **другая** ось «быстроты», не beginner. |
| 3. Наглядная система координат | **Нет** | Целевой клиент не нуждается в карте «какие бывают ИИ». Ему нужна карта «куда вписать AI в маркетинг моего бизнеса». Это **корпоративная** карта, не категориальная. |

**Diagnostic**: GRO «Интенсив» **не проходит** beginner-тест ни по одной оси из трёх — это **корректное состояние** для high-ticket B2B-формата. **Anti-positioning**: не использовать рекламные сообщения beginner-уровня («без программирования», «первый результат за вечер», «карта всех нейросетей»). Корректные сообщения: «вашему бизнесу нужна AI-маркетинговая стратегия, которую внедрит ваша команда» — это **decision-фаза corporate buyer**, не **awareness-фаза beginner**.

**Подтверждение из Pressfeed-обзора 2** (см. [[sources/2026-05-19-pressfeed-kakie-kursy-novichku-baranova-placement]]): рынок массовых beginner-курсов имеет focal-school (Барановой), активно продвигаемого через Pressfeed paid-placement (см. [[evolving/content-trends/pressfeed-paid-placement-ai-edu-pattern]]). Это значит **broad-funnel внимание** в нише AI-обучения занято — GRO не должен пытаться выиграть SEO/PR в этой нише напрямую (high cost-of-attention).

### Content scope из Pressfeed 2026-05-18 (обзор 15+ инструментов)

Второй Pressfeed-обзор «Нейросеть для курсов: 15+ доступных в РФ сервисов» ([[sources/2026-05-19-pressfeed-ai-tools-for-online-courses-15]]) даёт **готовый content scope** для GRO «Интенсива», если содержательный фокус — обучение AI-инструментам для маркетинга. Семь этапов = семь возможных дневных модулей (см. [[canon/marketing-frameworks/ai-course-production-conveyor-7-stages]]):

| День | Этап | Класс инструмента | Operational outcome для участника |
|---|---|---|---|
| 1 | Структура и контент | LLM + поисковой ассистент | Структура собственного контент-плана/курса на 90 дней |
| 2 | Задания и тесты | LLM | Готовый банк сценариев для маркетинговых задач (тесты для клиентов, кейсы для продаж) |
| 3 | Иллюстрации | Image-generators | Серия консистентных авторских иллюстраций для бренда |
| 4 | Презентации | AI-Presentation-Builder | Готовый шаблон корпоративной презентации с AI-генерируемыми вариантами |
| 5 | Видеоуроки (4 sub-этапа) | TTS + text-to-video + AI-editor | Готовый видеоурок/promo-видео для бренда |
| 6 | Доп. материалы | LLM | FAQ-секция / чек-листы / lead-magnet'ы из существующего контента |
| 7 | Интеграция в pipeline | Все вместе | Собственный production-pipeline для команды + ROI-план |

**Anti-pattern:** «дать обзор 30 нейросетей за день» (Pressfeed-style mass-market). **Pro-pattern:** один день = один этап с глубокой практикой на 2–3 инструментах + ручная работа на собственном бизнесе участника.

**Обязательный модуль про ограничения AI** — каждый день включает гигиенический блок из 3 ограничений (галлюцинации, шаблонность, правовая серость), см. [[canon/marketing-frameworks/ai-content-3-limitations-pressfeed]]. Без него интенсив провалится по критерию 5 (takeaway-сценарий): участники выйдут с pipeline, который ломается на проде без human-checkpoint'ов.

## Связанные страницы

- [[canon/product-knowledge/gro-app-overview]] — основной (подписочный) продукт GRO, на котором built интенсив-аудитория
- [[canon/product-knowledge/gro-pricing]] — публичная цена подписки (комплементарный ценовой контур)
- [[canon/target-audience/gro-segments]] — три broad-funnel сегмента GRO; intensive ICP — sub-segment Сегмента 2 (предприниматели в росте) с маркетинговой функцией
- [[canon/positioning/gro-value-proposition]] — общее позиционирование GRO, под которое подведён и интенсив
- [[canon/product-knowledge/gro-team]] — команда, потенциальные ведущие интенсива
- [[canon/marketing-frameworks/premium-perception-through-price]] — рамка про high-ticket pricing как маркетинговый сигнал
- [[canon/marketing-frameworks/ai-intensive-format-criteria]] — 5 критериев качественного AI-интенсива (отраслевая рамка для дифференциации GRO)
- [[canon/marketing-frameworks/beginner-edu-3-differentiators]] — рамка beginner-уровня (GRO «Интенсив» не должен по ней позиционироваться)
- [[canon/marketing-frameworks/ai-course-production-conveyor-7-stages]] — 7-этапная методология production курса (готовый content-scope для интенсива)
- [[evolving/content-trends/ru-ai-course-production-stack-2026]] — RU-tool inventory под 7 этапов (готовый toolset для программы)
- [[canon/marketing-frameworks/ai-content-3-limitations-pressfeed]] — 3 ограничения AI в обучающем контенте (обязательный гигиенический модуль)
- [[evolving/industry-trends/ru-ai-intensive-courses-landscape-2026]] — RU-landscape массовых AI-интенсивов, на фоне которых позиционируется GRO «Интенсив»
- [[evolving/content-trends/pressfeed-paid-placement-ai-edu-pattern]] — paid-placement пейзаж в нише AI-edu (GRO как корпоративный игрок vs Барановой как mass-market focal)
- [[canon/marketing-frameworks/spiridonov-three-engagement-formats]] — близкий по cost-структуре формат стратегической сессии 2–3 дней
